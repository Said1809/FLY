...
| Тестовых файлов | 10 |
| Тест-кейсов | 25 |
| **Покрытие строк (LINE)** | **87,7 %** (порог методички — > 40 %) |
| Покрытые модули | controller, entity, model, framework, TaskCard |

Покрытые файлы: `TaskController`, `WeekController`, `InMemoryTaskRepository`,
`SettingsModel`, `WeekCalculator`, `DateFormatter`, `Task`, `Week`, `TaskCard`.

Для повторного отчёта покрытия:

```bash
flutter test --coverage
# локально: coverage/lcov.info
```

Сохранённая копия отчёта для сдачи:
[coverage-report/lcov.info](coverage-report/lcov.info).
